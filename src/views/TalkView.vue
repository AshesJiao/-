<template>
    <div class="talk">
        <talk-detail :detail='detail' v-show='isShow' @emit-isShow="setIsShow"></talk-detail>
        <div class="header">
        ...
        <div class="talkWrapper" ref='talkContent'>
            <ul>
                <h2>最热话题</h2>
                <li class="talkItem" v-for="(item,index) in talkData" :key="index">
...
export default {
    ...
    methods:{
       ...
       addCount(index) { // 添加点赞功能
        // console.log(this.talkData[index])
        if (this.talkData[index].isPraise) {
          // 删除 对应的 点赞的属性 数字--
          this.$delete(this.talkData[index], 'isPraise', false)
          this.talkData[index].concernCount--
        } else {
          // 往对象中添加显示对应的 点赞的属性 数字++
          this.$set(this.talkData[index], 'isPraise', true)
          this.talkData[index].concernCount++
        }
      },
        showShare(){
